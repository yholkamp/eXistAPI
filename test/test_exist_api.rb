...
class TestExistAPI < Test::Unit::TestCase
  ...
  def setup
    ...
    @api.create_collection(@col_path)
    source = File.read("./test/cities.xml")
    @api.store_resource(source, @col_path+"/cities.xml")
  end
  ...
  def test_00_create_connection()
    ...
    assert(@api.exists_collection?(@col_path))
  end
  
  def test_01_create_collection
    path = "/db/testcollection/newcollection"
    @api.create_collection(path)
    assert(@api.exists_collection?(path))
  ...
  def test_02_get_collection
    col = @api.get_collection(@col_path)
    assert_not_nil(col)
  end
  
  def test_03_exists_collection?()
    assert(@api.exists_collection?(@col_path))
    col_path = "/db"
    assert(@api.exists_collection?(col_path), '"'+col_path+"\" should exist and doesnt.")
    col_path = "db"
    assert(@api.exists_collection?(col_path), '"'+col_path+"\" should exist and doesnt.")
    @api.create_collection("db/nexttest")
